	...
	run: async function(message, args) {
		...
		try {
			...
			for(let shardID = 0; shardID < this.client.shard.count; shardID++) {
				list += `│ ${shardID}${' '.repeat(8 - shardID.toString().length)} │ ${shardPings[shardID]}ms${' '.repeat(7 - shardPings[shardID].toString().length)} │ ${shardGuilds[shardID]}${' '.repeat(6 - shardGuilds[shardID].toString().length)} │ ~${shardUsers[shardID]}${' '.repeat(6 - shardUsers[shardID].toString().length)} │\n`;
			}
			...
			message.channel.send(`:x: Cannot get shard data: \`${err.message}\``);
