...
module.exports = {
	...
	run: async function (message, args, argsString) {
		if (argsString && isNaN(argsString)) return this.commandHandler.invalidArguments(message);

		const lineNr = argsString ? parseInt(argsString) : Math.floor(Math.random() * script.length);
