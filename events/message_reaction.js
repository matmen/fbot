module.exports = function () {
	...
	this.client.on('message', async(message) => {
		...
		const isBlacklisted = await this.utils.queryDB('SELECT * FROM blacklists WHERE (type = \'server\' AND id = $1) OR (type = \'channel\' AND id = $2) OR (type = \'user\' AND id = $3)', [message.guild ? message.guild.id : message.channel.id, message.channel.id, message.author.id]);
